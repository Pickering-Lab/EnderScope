...
class Manualgui:
    def __init__(self, master=None):
        ...
        self.fram_main.configure(height=1200, width=1200)
        ...
        disablesteppers = ttk.Button(self.fram_main)
        disablesteppers.configure(text='Disable Steppers')
        disablesteppers.grid(column=8, row=6)
        disablesteppers.configure(command=self.disable)
        ...
        self.camera.resolution = (960, 730)
        self.camera.framerate = 24
       
        # create array to store camera stream
        self.raw_capture = picamera.array.PiRGBArray(self.camera, size=(960, 730))
    ...
    def disable(self):
        disable = b'M18\n'
        ser.write(disable)
        messagebox.showwarning("Warning", "Steppers are now disabled. Click any XYZ button to enable steppers.")
